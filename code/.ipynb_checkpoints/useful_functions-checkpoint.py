...
def tfidf_markers(adata, groupby, groupvalue, layer):
    #do TFIDF MARKERS
    from scipy import sparse
    mat = adata.layers[layer]
    dat = sparse.csr_matrix(mat) #make the matrix
    ...
    n = universe_len - ntot
    N = target_len
    pvals = stats.hypergeom.cdf(q, m, n, N) 
    def FDR(x):
        """
...
def get_markers(adata, clusters, layer):
    pbar = ProgressBar()
    markers_list = [] #iterate this over a list...
    index_use = list(adata.obs[clusters].cat.categories)
    for i in pbar(list(index_use)):
        mark = tfidf_markers(adata[:,adata.var_names.values[adata.var.highly_variable]], clusters, i, layer)
